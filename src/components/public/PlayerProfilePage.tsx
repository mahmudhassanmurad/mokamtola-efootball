  ...
  if (!player || !profile) {
    return (
      <div className="text-center py-20 bg-slate-900/50 border border-slate-800 rounded-3xl p-8">
        ...
        <p className="text-sm text-slate-400 mt-2">The selected player could not be located in the tournament database.</p>
        <button
    ...
    <div className="space-y-6 pb-12">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2">
              <span className="px-2.5 py-0.5 text-[11px] font-bold uppercase tracking-wider rounded-full bg-emerald-500/20 text-emerald-400 border border-emerald-500/30">
                {player.status === 'active' ? 'Active Competitor' : 'Suspended'}
              </span>
            ...
            <p className="text-sm sm:text-base font-semibold text-emerald-400 mt-1">
              {player.teamName} <span className="text-slate-500 font-normal">• {player.fullName}</span>
            </p>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-4 text-center">
          ...
          <span className="text-[10px] text-slate-500">{profile.wins}W - {profile.draws}D - {profile.losses}L</span>
        </div>
        ...
        <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-4 text-center">
          ...
          <span className="text-[10px] text-emerald-500/80">{profile.wins} victories</span>
        </div>

        <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-4 text-center">
          <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider block">Total Points</span>
          <span className="text-2xl font-black text-amber-400 font-['Chakra_Petch',sans-serif] mt-1 block">
            {profile.points}
          </span>
          <span className="text-[10px] text-slate-500">League PTS</span>
        </div>

        <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-4 text-center">
          <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider block">Goals Scored</span>
          <span className="text-2xl font-black text-sky-400 font-['Chakra_Petch',sans-serif] mt-1 block">
            {profile.goalsScored}
          </span>
          <span className="text-[10px] text-slate-500">
            {profile.matchesPlayed > 0 ? (profile.goalsScored / profile.matchesPlayed).toFixed(2) : 0}/match
          </span>
        </div>

        <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-4 text-center">
          <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider block">Goal Diff</span>
          <span className={`text-2xl font-black font-['Chakra_Petch',sans-serif] mt-1 block ${
            profile.goalDifference > 0 ? 'text-emerald-400' : profile.goalDifference < 0 ? 'text-rose-400' : 'text-slate-300'
          }`}>
            {profile.goalDifference > 0 ? `+${profile.goalDifference}` : profile.goalDifference}
          </span>
          <span className="text-[10px] text-slate-500">{profile.goalsConceded} conceded</span>
        </div>

        <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-4 text-center">
          <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider block">Clean Sheets</span>
          <span className="text-2xl font-black text-teal-400 font-['Chakra_Petch',sans-serif] mt-1 block">
            {profile.cleanSheets}
          </span>
          <span className="text-[10px] text-slate-500">Zero conceded</span>
        </div>
      </div>

      {/* Goal Scoring & Assists Record */}
        ...
        <div className="bg-slate-900/80 border border-slate-800 rounded-3xl p-6">
          <div className="flex items-center gap-2.5 pb-4 border-b border-slate-800">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">{profile.goalsScored} total goals in tournament play</p>
            </div>
          </div>

          <div className="mt-4">
            {profile.goalsByMinute.length === 0 ? (
              <p className="text-xs text-slate-500 py-6 text-center">No individual goal timestamps recorded yet.</p>
            ) : (
              <div className="space-y-2">
                {profile.goalsByMinute.map((g, i) => (
                  <div key={i} className="flex items-center justify-between p-2.5 bg-slate-950/60 rounded-xl border border-slate-800/80 text-xs">
                    <span className="flex items-center gap-2 text-slate-200">
                      <span className="text-amber-400 font-mono font-bold">{g.minute}'</span>
                      <span>⚽ Goal scored</span>
                      {g.isPenalty && <span className="text-[10px] bg-amber-950/60 text-amber-400 px-1.5 rounded font-semibold">(Penalty)</span>}
                    </span>
                    <span className="text-slate-500 text-[11px]">Match #{g.matchId.replace('match-', '')}</span>
                  </div>
                ))}
              </div>
            )}
        ...
        <div className="bg-slate-900/80 border border-slate-800 rounded-3xl p-6">
          <div className="flex items-center gap-2.5 pb-4 border-b border-slate-800">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">{profile.assists} assists provided</p>
            </div>
          </div>

          <div className="mt-4">
            {profile.assistEvents.length === 0 ? (
              <p className="text-xs text-slate-500 py-6 text-center">No assists logged for this player yet.</p>
            ) : (
              <div className="space-y-2">
                {profile.assistEvents.map((a, i) => (
                  <div key={i} className="flex items-center justify-between p-2.5 bg-slate-950/60 rounded-xl border border-slate-800/80 text-xs">
                    <span className="flex items-center gap-2 text-slate-200">
                      <span className="text-sky-400 font-mono font-bold">{a.minute}'</span>
                      <span>👟 Assist to {a.scorerName}</span>
                    </span>
                    <span className="text-slate-500 text-[11px]">Match #{a.matchId.replace('match-', '')}</span>
                  </div>
                ))}
              </div>
            )}
      ...
      <div className="bg-slate-900/80 border border-slate-800 rounded-3xl p-6">
        <div className="flex items-center justify-between pb-4 border-b border-slate-800">
          <div>
            ...
            <p className="text-xs text-slate-400">Chronological tournament appearances</p>
          </div>
        </div>

        <div className="mt-4 space-y-3">
          {profile.recentMatches.length === 0 ? (
            <p className="text-xs text-slate-500 py-6 text-center">No completed matches recorded yet.</p>
          ) : (
            profile.recentMatches.map((m) => {
              ...
              const isCompleted = m.status === 'completed' && myScore !== null && oppScore !== null;
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3">
                    <span className={`px-2 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider ${badgeColor}`}>
                      {outcome}
                    </span>
                    <div>
                      <span className="text-[11px] text-slate-400 uppercase font-semibold">{m.roundName}</span>
                      <div className="flex items-center gap-2 mt-0.5">
                        <span className="font-bold text-white">vs {opp?.displayName || 'Opponent'}</span>
                        <span className="text-[11px] text-slate-400">({opp?.teamName})</span>
                      </div>
                    </div>
                  </div>
